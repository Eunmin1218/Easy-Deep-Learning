...
Q. 왜 GD에서 데이터가 많아질 경우 한 번의 파라미터 업데이트에 상당한 시간이 소요되는가? L 식은 처음 한 번만 계산하면 되는 것 아닌가? 선형 회귀에서 결국 L식이 이차함수가 되어 처리가 쉬울 것 같은데 의문이다.  
A. 각 데이터 포인트에 대해 계산을 하기 때문

Q. 기울기를 구할 때 왜 각 데이터 포인트에 대해 계산을 해야 하는가?  
A. (챗 GPT 답변) 기울기 계산을 위해 각 데이터 포인트의 기여를 반영해야 하기 때문입니다. 손실 함수 L(θ)는 모든 데이터 포인트에 대한 오차의 제곱을 합산한 값입니다. 따라서 각 데이터 포인트가 모델의 가중치 업데이트에 미치는 영향을 정확하게 반영하려면 각 데이터 포인트에 대해 기울기를 계산하고 그 기여를 모두 합산해야 합니다.

즉, 기울기는 각 데이터가 모델에 미치는 영향을 합산하는 과정입니다. 하나의 식을 편미분하여 대입하는 것만으로는, 전체 데이터에 대한 기여를 정확히 반영할 수 없습니다. 데이터가 하나하나가 모델에 미치는 영향을 구하고 이를 모두 더해야 최종적인 기울기를 구할 수 있습니다.
